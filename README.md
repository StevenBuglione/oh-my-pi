<p align="center">
  <img src="https://github.com/can1357/oh-my-gpt/blob/main/assets/hero.png?raw=true" alt="omg">
</p>

<p align="center">
  <strong>A coding agent with the IDE wired in.</strong>
  <strong><a href="https://omg.sh">omg.sh</a></strong>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/@oh-my-gpt/gpt-coding-agent"><img src="https://img.shields.io/npm/v/@oh-my-gpt/gpt-coding-agent?style=flat&colorA=222222&colorB=CB3837" alt="npm version"></a>
  <a href="https://github.com/can1357/oh-my-gpt/blob/main/packages/coding-agent/CHANGELOG.md"><img src="https://img.shields.io/badge/changelog-keep-E05735?style=flat&colorA=222222" alt="Changelog"></a>
  <a href="https://github.com/can1357/oh-my-gpt/actions"><img src="https://img.shields.io/github/actions/workflow/status/can1357/oh-my-gpt/ci.yml?style=flat&colorA=222222&colorB=3FB950" alt="CI"></a>
  <a href="https://github.com/can1357/oh-my-gpt/blob/main/LICENSE"><img src="https://img.shields.io/github/license/can1357/oh-my-gpt?style=flat&colorA=222222&colorB=58A6FF" alt="License"></a>
...
```sh
curl -fsSL https://omg.sh/install | sh
```

**Bun (recommended)**

```sh
bun install -g @oh-my-gpt/gpt-coding-agent
```

**Windows (PowerShell)**

```powershell
irm https://omg.sh/install.ps1 | iex
```

**Pinned versions (mise)**

```sh
mise use -g github:can1357/oh-my-gpt
```
...
Edits that land on the first attempt. Reads that summarize files instead of dumping their content. Searches that return instantly. Pick any model — omg will get it right.
...
Originally built on [Mario Zechner](https://github.com/mariozechner)'s wonderful [Pi](https://github.com/badlogic/pi-mono), omg adds everything you're missing.
...
![omg TUI: a single eval session with `[1/2] pandas describe` (Python) printing a real DataFrame.describe() table, followed by `[2/2] top scorer` (JavaScript) running a reduce. Footer: 'Both kernels ran in one session.'](https://omg.sh/captures/eval.webp)

### 02 · LSP wired into every write

Ask for a rename and you get a rename. The call goes through workspace/willRenameFiles, so re-exports, barrel files, and aliased imports update before the file moves. Everything your IDE knows, the agent knows.

![omg TUI: `LSP references` returns five hits across three files for the symbol `formatBytes`, then `LSP rename` applies the change with edits to format.ts/report.ts/cli.ts, then a `Search formatBytes 0 matches` confirmation. Final line: 'Rename complete. Five edits across three files…'.](https://omg.sh/captures/lsp.webp)
...
![omg TUI: a live lldb-dap session against a native binary at /tmp/omg-native/demo. Adapter=lldb-dap, Status=stopped, Frame=xorshift32, Instruction pointer 0x10000055C, Location demo.c:6:10. Debug scopes and Debug variables cards show locals (x = 57351) and the agent confirms the math: x went from 7 → 57351 (= 7 ^ (7<<13)).](https://omg.sh/clips/dap-poster.webp)

_[Watch the capture ↗](https://omg.sh/clips/dap.mp4)_
...
![omg TUI: web_search returns 10 ranked Perplexity sources for inference-time compute scaling, the agent picks an arxiv paper, calls read https://arxiv.org/pdf/2604.10739v1, and summarizes the paper's headline result with real numbers.](https://omg.sh/clips/web-poster.webp)

_[Watch the capture ↗](https://omg.sh/clips/web.mp4)_

### 07 · Unapologetically native. Even on Windows.

Other agents shell out to rg, grep, find, and bash. On many machines those binaries don't exist, and on the ones where they do, every call costs a fork-exec round-trip. omg links the real implementations into the process. ripgrep, glob, find: in-process. brush is the bash, with sessions that survive across calls. The same omg binary runs on macOS, Linux, and Windows — no WSL bridge.
...
![omg TUI reading pr://can1357/oh-my-gpt/1063 and then /diff/1, showing hunk headers, added lines, and a [MODIFIED] (+12 -0) summary.](https://omg.sh/captures/pr.webp)
...
![omg TUI: ✓ Read src/session.ts (⚠ 1 conflict), then ✓ Write conflict://1 · 1 line with content @theirs, then a confirmation 'Resolved.'](https://omg.sh/clips/conflict-poster.webp)

_[Watch the capture ↗](https://omg.sh/clips/conflict.mp4)_
...
![omg TUI: ✓ AST Edit: console.log($X) (proposed) 3 replacements · 1 file, then ✓ Accept: 3 replacements in 1 file (AST Edit), followed by 'Applied 3 replacements in src/auth.ts.'](https://omg.sh/clips/codemod-poster.webp)

_[Watch the capture ↗](https://omg.sh/clips/codemod.mp4)_
...
![omg TUI driving the browser tool against DuckDuckGo](https://omg.sh/captures/browser.webp)
...
[Full reference →](https://omg.sh/docs/tools)
...
- **Custom providers** — Declare anything that speaks `openai-completions`, `openai-responses`, `openai-codex-responses`, `azure-openai-responses`, `anthropic-messages`, `google-generative-ai`, or `google-vertex` in `~/.omg/agent/models.yml`.
...
Full provider & routing reference at [omg.sh/docs/providers](https://omg.sh/docs/providers).
...
[`web_search` reference ↗](https://omg.sh/docs/tools#web_search)
...
- Crates: `gpt-natives`, `pi-shell`, `pi-ast`
...
Same engine, four wrappers. `omg` runs the TUI. `omg -p` answers a single prompt and exits. The Node SDK embeds the session in your process. `omg --mode rpc` and `omg acp` hand the wheel to another program over stdio.
...
![omg TUI: the ask tool renders an option picker with three choices, a (Recommended) badge on the first, and 'up/down navigate · enter select · esc cancel' footer.](https://omg.sh/captures/ask.webp)

### SDK — embed in Node

`@oh-my-gpt/gpt-coding-agent`
...
```ts
import { ModelRegistry, SessionManager, createAgentSession, discoverAuthStorage } from "@oh-my-gpt/gpt-coding-agent";
...
Full reference: [omg.sh/docs/sdk](https://omg.sh/docs/sdk).

## A harness worth keeping is one you _don't_ outgrow.

Pick it up at **[omg.sh](https://omg.sh)**.

omg is a fork of [Pi](https://github.com/badlogic/pi-mono) by [Mario Zechner](https://github.com/mariozechner), rewritten as a coding-first surface: sessions, subagents, slash commands, extensions — all TypeScript, all MIT, all on [GitHub](https://github.com/can1357/oh-my-gpt). Shape it from config, hook it from outside, or read the source when you need to.
...
On first run omg inherits whatever is already on disk: rules, skills, and MCP servers from `.claude`, `.cursor`, `.windsurf`, `.gemini`, `.codex`, `.cline`, `.github/copilot`, and `.vscode`. No migration script.

### Extensibility

Ask omg to write the piece you're missing, then `/reload-plugins`. Keep it local, ship it in a `marketplace`, or publish it to npm.

## Philosophy

omg is a fork of [pi-mono](https://github.com/badlogic/pi-mono) by [Mario Zechner](https://github.com/mariozechner), extended with a batteries-included coding workflow.
...
| Package                                                   | Description                                                                |
| --------------------------------------------------------- | -------------------------------------------------------------------------- |
| **[@oh-my-gpt/gpt-ai](packages/ai)**                        | Multi-provider LLM client with streaming and model/provider integration    |
| **[@oh-my-gpt/gpt-agent-core](packages/agent)**             | Agent runtime with tool calling and state management                       |
| **[@oh-my-gpt/gpt-coding-agent](packages/coding-agent)**    | Interactive coding agent CLI and SDK                                       |
| **[@oh-my-gpt/gpt-tui](packages/tui)**                      | Terminal UI library with differential rendering                            |
| **[@oh-my-gpt/gpt-natives](packages/natives)**              | N-API bindings for grep, shell, image, text, syntax highlighting, and more |
| **[@oh-my-gpt/omg-stats](packages/stats)**                 | Local observability dashboard for AI usage statistics                      |
| **[@oh-my-gpt/gpt-utils](packages/utils)**                  | Shared utilities (logging, streams, dirs/env/process helpers)              |
| **[@oh-my-gpt/swarm-extension](packages/swarm-extension)** | Swarm orchestration extension package                                      |

### Rust Crates

| Crate                                                         | Description                                                                                         |
| ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **[gpt-natives](crates/gpt-natives)**                           | Core Rust native addon (N-API `cdylib`) used by `@oh-my-gpt/gpt-natives`; aggregates the crates below |
| **[pi-shell](crates/pi-shell)**                               | Embedded shell / PTY / process management split out of `gpt-natives` (wraps `brush-*`)               |
...
- [omg.sh](https://omg.sh)
- [GitHub](https://github.com/can1357/oh-my-gpt)
- [Changelog](https://github.com/can1357/oh-my-gpt/blob/main/packages/coding-agent/CHANGELOG.md)
- [npm](https://www.npmjs.com/package/@oh-my-gpt/gpt-coding-agent)
- [Discord](https://discord.gg/4NMW9cdXZa)
- [MIT](https://github.com/can1357/oh-my-gpt/blob/main/LICENSE)
