# OMG Coding Agent Installer for Windows
# Usage: irm https://raw.githubusercontent.com/can1357/oh-my-gpt/main/scripts/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/can1357/oh-my-gpt/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/can1357/oh-my-gpt/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/can1357/oh-my-gpt/main/scripts/install.ps1))) -Source -Ref v3.20.1
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/can1357/oh-my-gpt/main/scripts/install.ps1))) -Source -Ref main
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/can1357/oh-my-gpt/main/scripts/install.ps1))) -Binary -Ref v3.20.1
...
$Repo = "can1357/oh-my-gpt"
$Package = "@oh-my-gpt/gpt-coding-agent"
$InstallDir = if ($env:PI_INSTALL_DIR) { $env:PI_INSTALL_DIR } else { "$env:LOCALAPPDATA\omg" }
$BinaryName = "omg-windows-x64.exe"
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".omg\agent"
        ...
        if ($bashPath) {
            ...
            Write-Host "  OMG requires a bash shell on Windows. Options:" -ForegroundColor Yellow
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("omg-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "✓ Installed omg via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'omg' to get started!"
}
...
function Install-Binary {
    ...
    $OutPath = Join-Path $InstallDir "omg.exe"
    Invoke-WebRequest -Uri $BinaryUrl -OutFile $OutPath

    Write-Host ""
    Write-Host "✓ Installed omg to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'omg' to get started!"
    } else {
        Write-Host "Run 'omg' to get started!"
    }
}
