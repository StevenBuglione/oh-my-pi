...
for pkg in "${PACKAGES[@]}"; do
    ...
    echo "Dependencies:"
    jq '.dependencies | to_entries[] | select(.value | startswith("@oh-my-gpt") or startswith("workspace"))' package.json 2>/dev/null || true
...
RUN verdaccio --config /root/.config/verdaccio/config.yaml &>/dev/null & \
    sleep 3 && \
    bun add @oh-my-gpt/gpt-coding-agent --registry http://localhost:4873 && \
    pkill -f verdaccio
