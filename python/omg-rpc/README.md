...
Typed Python bindings for the `omg --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `omg --mode rpc` flags such as thinking level,
...
```python
from omg_rpc import MessageUpdateEvent, RpcClient
...
from omg_rpc import RpcClient, host_tool
...
```python
from omg_rpc import RpcClient, host_uri
...
```python
from omg_rpc import assistant_text, assistant_text_with_thinking
