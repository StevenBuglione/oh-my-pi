...
import httpx
import pytest
from omg_rpc import HostToolContext, RpcCommandError
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    monkeypatch.setenv("ROBOMG_GH_PROXY_HMAC_KEY", "secret-proxy")
    ...
    assert env["ROBOMG_GH_PROXY_HMAC_KEY"] == ""
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bindings.workspace.root / ".omg-xdg" / "cache" / "bun-install")
    assert env["XDG_CACHE_HOME"] == str(bindings.workspace.root / ".omg-xdg" / "cache")
    assert env["TMPDIR"] == str(bindings.workspace.root / ".omg-tmp")
    ...
    assert (bindings.workspace.root / ".omg-tmp").is_dir()
...
def test_run_repo_command_uses_slot_identity_kwargs(
    ...
    assert kwargs["env"]["BUN_INSTALL_CACHE_DIR"].endswith("/.omg-xdg/cache/bun-install")
