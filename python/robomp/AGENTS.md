...
`roboomp` is a self-hosted GitHub triage-and-fix bot that drives [`omg --mode rpc`](https://github.com/can1357/oh-my-gpt) as a subprocess. On every issue opened in an allowlisted repository it classifies the issue, applies labels, then branches into one of: reproduce → fix → PR (`bug` / `documentation`), single-comment answer (`question`), single thoughtful comment (`enhancement` / `proposal`), or brief comment (`invalid` / `duplicate`). Follow-up comments and PR review comments resume the same omg session so the agent keeps its prior reasoning. If the orchestrator restarts mid-task, the dispatcher resumes the same session via `omg --continue` from the per-issue `session_dir`, so an interrupted task re-enters its prior reasoning instead of restarting from scratch. The orchestrator runs as a single FastAPI process inside Docker with SQLite-backed durable event state.

## Architecture & Data Flow

Webhook → durable queue → async dispatcher → per-issue git worktree → omg RPC subprocess + host tools.
...
4. `queue.WorkerPool._dispatch_loop` atomically claims `state='queued'` rows under `BEGIN IMMEDIATE`, guarded by an in-process `_inflight` set keyed by `(owner, repo, number)` to serialize per-issue work. Cap: `ROBOMG_MAX_CONCURRENCY` (default 8).
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `omg --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked model from `ROBOMG_MODEL` (CSV pool). When `<session_dir>/*.jsonl` already exists the worker passes `--continue`, so both follow-up events and crash-restarted events resume the same session.
7. Inside the subprocess the agent uses **built-in omg tools** (read/edit/write/bash/lsp, scoped to the worktree) and **host tools** from `host_tools.py` (the only surface allowed to mutate GitHub or write audit rows).
...
- `tests/` — pytest suite. `test_worker_smoke.py` is gated on `ROBOMG_INTEGRATION=1`.
- `data/` — runtime state (sqlite + WAL, `workspaces/`, `logs/`). Never committed.
- `/Dockerfile` (pi root) — produces `oh-my-gpt/pi:dev` (pi runtime image: python + bun + rustup + gpt-natives + omg_rpc + `/usr/local/bin/omg` shim + the full pi source under `/pi`). Stages: `natives-builder` → `wheel-builder` → `pi-base` → `pi-runtime` (default). Built via `bun run omg:image`. Robomp's image extends `pi-base` via `FROM ${OMG_BASE}` in `/Dockerfile.robomp`.
...
```
bun run test:py                   # pytest -x python/omg-rpc/tests python/robomp/tests
bun run robomp:test:integration   # ROBOMG_INTEGRATION=1, requires omg on PATH
...
bun run omg:run                    # docker run -it oh-my-gpt/pi:dev (smoke-test the shim)
bun run robomp:build              # omg:image (if pi changed) + docker compose build
...
HTTP / sqlite / webhook inspection is unaliased — use `curl http://localhost:${ROBOMG_BIND_PORT:-8080}/{healthz,readyz,events,issues}` and `docker compose --project-directory python/robomp exec robomp sqlite3 /data/robomp.sqlite` directly.
...
- **Async style**: FastAPI handlers and `queue.WorkerPool` are async. `worker.run_task` is **synchronous** and runs in a worker thread because `omg-rpc` is blocking — keep it that way; don't try to async it. CLI commands wrap with `asyncio.run`.
- **Config**: `pydantic-settings` `Settings` in `config.py` with `ROBOMG_*` env prefix (e.g. `ROBOMG_MAX_CONCURRENCY`, `ROBOMG_REPO_ALLOWLIST`). Access only via `get_settings()` (`@cache` singleton). Tests must call `reset_settings_cache()` after mutating env.
...
- `src/worker.py` — synchronous omg RPC driver, prompt assembly via `persona`.
...
- `/Dockerfile.robomp` (pi root) — robomp's image. `FROM ${OMG_BASE}` (default `oh-my-gpt/pi:dev`), adds the SolidJS dashboard bundle, the robomp Python package, and the `robomp-entrypoint` shim. Tini entrypoint, exposes `8080`, `VOLUME /data`. The toolchain (python + bun + rustup + gpt-natives + omg_rpc + `omg` shim) comes from `pi-base` — no duplication in this file.
- `docker-compose.yml` — `build.args.OMG_BASE`, mounts `$OMG_ROOT:/work/pi:ro`, `./data:/data`, `~/.omg/agent/models.container.yml:ro` (mapped to `models.yml` inside the container — kept separate from the host's `~/.omg/agent/models.yml` so the host omg doesn't pick up gateway routing intended only for the container), `extra_hosts: llm-gateway.internal:host-gateway`.
- `entrypoint.sh` — validates `OMG_ROOT`, creates `/data/{workspaces,logs}` + build caches.
...
- **Container runtime**: Docker Compose v2. The image embeds Bun 1.3.14 + a rustup launcher and exposes `omg` via a `/usr/local/bin/omg` shim; `ROBOMG_OMG_COMMAND=omg` should not need changing.
- **Required env** (set in `.env`, see `.env.example`): `GITHUB_WEBHOOK_SECRET`, `ROBOMG_BOT_LOGIN`, `ROBOMG_GIT_AUTHOR_NAME`, `ROBOMG_GIT_AUTHOR_EMAIL`, `ROBOMG_REPO_ALLOWLIST`, plus model knobs (`ROBOMG_MODEL`, `ROBOMG_THINKING`, optional `ROBOMG_PROVIDER`) and rate-limit / concurrency / timeout overrides. **GitHub auth is mode-exclusive**: either set `ROBOMG_GH_PROXY_URL` + `ROBOMG_GH_PROXY_HMAC_KEY` (gh-proxy mode; PAT lives only in the sidecar container — the bundled compose default), or set `GITHUB_TOKEN` directly (single-process PAT mode). `Settings._validate_proxy_or_pat` rejects a `.env` that sets both.
- **OMG_ROOT resolution**: roboomp lives inside the oh-my-gpt monorepo at `python/robomp/`. `bun run omg:image` builds the parent monorepo (`../..`) as its docker build context to produce `oh-my-gpt/pi:dev`; `docker-compose.yml` extends that image via `OMG_BASE` and mounts the same parent path read-only at `/work/pi` for the orchestrator to see live source. Override `OMG_ROOT` only when pointing the build/mount at a different oh-my-gpt checkout. Inside the container the path is always `/work/pi`. Build invalidation stays bounded: Python-only edits in roboomp never trigger a natives recompile.
...
- **Fixtures** (`tests/conftest.py`):
  - `env` — `monkeypatch`-sets all required `ROBOMG_*` env vars and calls `reset_settings_cache()` before/after.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `ROBOMG_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `omg` on `PATH`. Don't enable it in default `bun run test:py`.
