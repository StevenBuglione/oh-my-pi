...
const outputPath = path.join(packageDir, "dist", "omg");
...
async function main(): Promise<void> {
	await runCommand([bunExe, "--cwd=../stats", "scripts/generate-client-bundle.ts", "--generate"]);
	try {
		await runCommand([bunExe, "--cwd=../natives", "run", "embed:native"]);
		try {
			...
			await runCommand([bunExe, "--cwd=../natives", "run", "embed:native", "--reset"]);
		}
	} finally {
		await runCommand([bunExe, "--cwd=../stats", "scripts/generate-client-bundle.ts", "--reset"]);
	}
}
