...
} from "@oh-my-gpt/gpt-coding-agent/system-prompt";
...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".omg");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".omg"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".omg", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".omg", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".omg", "SYSTEM.md"), "Project SYSTEM prompt");
