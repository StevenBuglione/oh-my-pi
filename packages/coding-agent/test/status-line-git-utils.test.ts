...
import {
	...
	parseDefaultBranch,
	parseGitHubRepo,
} from "@oh-my-gpt/gpt-coding-agent/modes/components/status-line/git-utils";

describe("parseGitHubRepo", () => {
	test("parses HTTPS URL", () => {
		expect(parseGitHubRepo("https://github.com/can1357/oh-my-gpt.git")).toBe("can1357/oh-my-gpt");
	});

	test("parses HTTPS URL without .git suffix", () => {
		expect(parseGitHubRepo("https://github.com/can1357/oh-my-gpt")).toBe("can1357/oh-my-gpt");
	});

	test("parses SSH scp-style URL", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/oh-my-gpt.git")).toBe("loftiskg/oh-my-gpt");
	});

	test("parses SSH scp-style URL without .git suffix", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/oh-my-gpt")).toBe("loftiskg/oh-my-gpt");
	});
