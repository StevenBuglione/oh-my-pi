...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@oh-my-gpt/gpt-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.omg/`), plus
//    the legacy non-XDG `~/.omg/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.omg/agent` or `$OMG_CODING_AGENT_DIR`)
//    Handles `OMG_CODING_AGENT_DIR` overrides that relocate the agent dir
//    (and therefore `agent/extensions/`) out from under the config root.
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/omg/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.omg` when
//    XDG_DATA_HOME resolves the plugins dir somewhere else.
//
// We deliberately do NOT use `pathIsWithin` from gpt-utils here: that helper
// resolves symlinks via `fs.realpathSync` on both sides, so a contributor who
// symlinks `~/.omg/agent/extensions/foo.ts` to a dotfiles repo would see the
