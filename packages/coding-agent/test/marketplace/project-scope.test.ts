 ...
 *
 * Note: helpers.ts imports @oh-my-gpt/gpt-natives (Rust addon via glob).
...
import type { InstalledPluginEntry } from "@oh-my-gpt/gpt-coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
	readInstalledPluginsRegistry,
	writeInstalledPluginsRegistry,
} from "@oh-my-gpt/gpt-coding-agent/extensibility/plugins/marketplace";
import {
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "omg-proj-scope-"));
	});
	...
	it("walk-up finds nearest .omg/ directory", async () => {
		// Layout: tmpDir/.omg/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .omg/ there.
		fs.mkdirSync(path.join(tmpDir, ".omg"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".omg", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .omg/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.omg/   +   tmpDir/sub/.omg/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.omg/, not climb further to tmpDir/.omg/.
		fs.mkdirSync(path.join(tmpDir, ".omg"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".omg"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".omg", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .omg/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .omg/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .omg/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .omg/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.omg/.../installed_plugins.json.
		...
		try {
			// Start from a tmpDir that has no .omg/ or .git/ of its own.
			const result = await resolveActiveProjectRegistryPath(tmpDir);
			// Must not resolve to the home-dir OMG registry.
			const homeOmpPath = path.join(homeDir, ".omg", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listClaudePluginRoots reads the user OMG registry. */
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "omg-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "omg-shadow-proj-"));

		// Create .omg/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".omg", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".omg", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".omg", "plugins", "installed_plugins.json");
	});
