 ...
 *
 * Each test isolates `OMG_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "@oh-my-gpt/gpt-coding-agent/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
} from "@oh-my-gpt/gpt-coding-agent/tools/github-cache";
...
beforeEach(async () => {
	...
	originalEnv = process.env.OMG_GITHUB_CACHE_DB;
	process.env.OMG_GITHUB_CACHE_DB = path.join(tempDir, "github-cache.db");
	resetCacheForTests();
});
...
afterEach(async () => {
	...
	if (originalEnv === undefined) {
		delete process.env.OMG_GITHUB_CACHE_DB;
	} else {
		process.env.OMG_GITHUB_CACHE_DB = originalEnv;
	}
	vi.restoreAllMocks();
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.OMG_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
