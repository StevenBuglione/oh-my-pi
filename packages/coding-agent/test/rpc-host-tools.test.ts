...
import type { AgentEvent } from "@oh-my-gpt/gpt-agent-core";
import { defineRpcClientTool, RpcClient } from "@oh-my-gpt/gpt-coding-agent/modes";
import { RpcHostToolBridge } from "@oh-my-gpt/gpt-coding-agent/modes/rpc/host-tools";
import type {
	RpcHostToolCallRequest,
	RpcHostToolCancelRequest,
	RpcHostToolUpdate,
} from "@oh-my-gpt/gpt-coding-agent/modes/rpc/rpc-types";
...
describe("RpcClient custom tools", () => {
	it("registers host custom tools and serves tool calls over the RPC transport", async () => {
		const scriptPath = path.join(os.tmpdir(), `omg-rpc-host-tools-${Date.now()}.js`);
