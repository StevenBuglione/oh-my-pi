...
import { executePython } from "@oh-my-gpt/gpt-coding-agent/eval/py/executor";
import type { KernelExecuteOptions, KernelExecuteResult } from "@oh-my-gpt/gpt-coding-agent/eval/py/kernel";
import { PythonKernel } from "@oh-my-gpt/gpt-coding-agent/eval/py/kernel";
...
describe("executePython (per-call)", () => {
	...
	it("returns a cancelled timeout result when kernel startup exceeds the deadline", async () => {
		...
		using tempDir = TempDir.createSync("@omg-python-executor-per-call-");
