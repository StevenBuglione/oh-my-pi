...
import { type ContextFile, contextFileCapability } from "@oh-my-gpt/gpt-coding-agent/capability/context-file";
import { resetSettingsForTest, Settings } from "@oh-my-gpt/gpt-coding-agent/config/settings";
import { initializeWithSettings, loadCapability } from "@oh-my-gpt/gpt-coding-agent/discovery";
...
describe("disabledExtensions runtime filtering", () => {
	...
	beforeEach(async () => {
		...
		tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "omg-disabled-ext-home-"));
		process.env.HOME = tempHomeDir;
		vi.spyOn(os, "homedir").mockReturnValue(tempHomeDir);
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "omg-disabled-ext-"));
		await fs.mkdir(path.join(tempDir, ".omg"), { recursive: true });
		await fs.writeFile(path.join(tempDir, ".omg", "AGENTS.md"), "# project instructions\n");
