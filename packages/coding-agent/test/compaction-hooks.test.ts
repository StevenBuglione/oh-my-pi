...
import {
	...
	type SessionEvent,
} from "@oh-my-gpt/gpt-coding-agent/extensibility/hooks";
import { theme } from "@oh-my-gpt/gpt-coding-agent/modes/theme/theme";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("Compaction hooks", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `omg-compaction-hooks-test-${Snowflake.next()}`);
