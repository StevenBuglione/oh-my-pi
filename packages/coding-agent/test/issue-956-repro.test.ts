...
import { getConfigRootDir, getProjectDir, setAgentDir, setProjectDir } from "@oh-my-gpt/gpt-utils";
...
describe("issue #956: interactive /mcp test", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "omg-issue-956-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "omg-issue-956-agent-"));
