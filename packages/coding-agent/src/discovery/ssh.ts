 ...
 * SSH JSON Provider
 *
 * Discovers SSH hosts from managed omg config paths and legacy root ssh.json files.
 * Priority: 5 (low, project/user config discovery)
 */
import * as path from "node:path";
import { getSSHConfigPath, tryParseJson } from "@oh-my-gpt/gpt-utils";
...
registerProvider(sshCapability.id, {
	...
	description: "Load SSH hosts from managed omg paths and legacy ssh.json/.ssh.json files",
	priority: 5,
	load,
});
