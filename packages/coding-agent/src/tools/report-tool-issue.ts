 ...
 * bundled endpoint (`dev.autoqaPush.endpoint`, default `qa.omg.sh`). Each
...
import { $env, $flag, getAgentDir, getInstallId, logger, VERSION } from "@oh-my-gpt/gpt-utils";
 ...
 * `~/.omg/agent/autoqa.db`. Idempotently runs schema creation, the
 * `pushed`-column migration, and index setup so every consumer — tool
 * execute path, manual `omg grievances push`, future debug scripts —
 ...
 *
 * Exported because the `omg grievances` CLI handlers need the migrated
...
/**
 * Optional per-flush controls. Used by `omg grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`omg grievances push`) where the
...
async function performFlush(db: Database, config: PushConfig, options: FlushOptions = {}): Promise<FlushResult> {
	...
	for (;;) {
		...
		const body = JSON.stringify({
			agent: { name: "omg", version: VERSION },
...
export function createReportToolIssueTool(session: ToolSession, activeBuiltinNames: readonly string[] = []): AgentTool {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			// at ~/.omg/agent/autoqa.db regardless of consent — they always
			// own their local data and can inspect or wipe it via `omg grievances`.
			...
			try {
				...
				if (db) {
					...
					//      `omg grievances push` or a future consent change.
