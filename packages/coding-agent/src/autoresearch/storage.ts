...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@oh-my-gpt/gpt-utils";
 ...
 *
 * Used to key per-project autoresearch state under `~/.omg/autoresearch/`.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.OMG_AUTORESEARCH_DB_DIR;
