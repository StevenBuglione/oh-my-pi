...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@oh-my-gpt/gpt-utils";
 ...
 * User-level: ~/.omg/agent, ~/.claude, ~/.codex, ~/.gemini
 * Project-level: .omg, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".omg", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.omg/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.omg/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.omg/agent/commands", source: ".omg", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.omg, .claude) - the nearest one found.
