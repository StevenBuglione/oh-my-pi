...
import { getAgentDir, getProjectDir, isEnoent } from "@oh-my-gpt/gpt-utils";
...
export async function installPlugin(packageName: string): Promise<InstalledPlugin> {
	...
	if (!(await pkgJson.exists())) {
		await pkgJson.write(JSON.stringify({ name: "omg-plugins", private: true, dependencies: {} }, null, 2));
	}
	...
	return {
		...
		manifest: pkg.omg || pkg.pi || { version: pkg.version },
