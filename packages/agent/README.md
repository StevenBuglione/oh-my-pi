...
Stateful agent with tool execution and event streaming. Built on `@oh-my-gpt/gpt-ai`.

## Installation

```bash
npm install @oh-my-gpt/gpt-agent
```

## Quick Start

```typescript
import { Agent } from "@oh-my-gpt/gpt-agent";
...
```typescript
declare module "@oh-my-gpt/gpt-agent" {
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@oh-my-gpt/gpt-ai`).
...
```typescript
import { Agent, streamProxy } from "@oh-my-gpt/gpt-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@oh-my-gpt/gpt-agent";
