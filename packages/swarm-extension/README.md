...
Multi-agent orchestration for oh-my-gpt. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full oh-my-gpt subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
omg-swarm path/to/swarm.yaml

# Background — survives terminal close:
nohup omg-swarm path/to/swarm.yaml \
...
### Inside oh-my-gpt (TUI)

Register the extension in your config (`~/.omg/config.json` or `.omg/config.json`):
...
Each agent is a full oh-my-gpt session. It can:
...
Any model configured in omg works. Set a swarm default and optionally override per agent:
...
src/swarm/
  ...
  executor.ts         Spawns agents via oh-my-gpt's runSubprocess
