...
- **Session log parsing**: Reads JSONL session logs from `~/.omg/agent/sessions/`
...
```typescript
import { getDashboardStats, syncAllSessions } from "@oh-my-gpt/omg-stats";
...
- **Session logs**: `~/.omg/agent/sessions/` (JSONL files)
- **Stats database**: `~/.omg/stats.db` (SQLite)
