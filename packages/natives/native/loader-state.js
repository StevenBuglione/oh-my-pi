...
/**
 * Native addon loader for `@oh-my-gpt/gpt-natives`.
 *
 ...
 *
 * Background (issue #823): `bun build --compile --define OMG_COMPILED=true`
 * substitutes the bare identifier `OMG_COMPILED`, NOT `process.env.OMG_COMPILED`,
...
function getNativesDir() {
	...
	if (xdgDataHome && fs.existsSync(path.join(xdgDataHome, "omg"))) {
		return path.join(xdgDataHome, "omg", "natives");
	}
	return path.join(os.homedir(), ".omg", "natives");
}
...
export function detectCompiledBinary({ embeddedAddon, env, importMetaUrl }) {
	...
	if (env && env.OMG_COMPILED) return true;
 ...
 * into the per-version cache directory (`~/.omg/natives/<version>/`) before loading.
 *
 * Windows-only safety net for `bun install -g` updates: when a previous `omg`
 * process is running, bun cannot overwrite the locked `.node` inside
 * `node_modules/@oh-my-gpt/gpt-natives/native/`, leaving an old binary next to a
 * newer `index.js` and producing `<sym> is not a function` crashes on the next
 * launch. Staging into the version-pinned cache:
 *   1. Gives every package version its own filesystem path, so concurrent omg
...
function validateLoadedBindings(ctx, bindings, candidate) {
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @oh-my-gpt/gpt-natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	if (ctx.isCompiledBinary) {
		...
		const downloadHints = ctx.addonFilenames
			.map(filename => {
				const downloadUrl = `https://github.com/can1357/oh-my-gpt/releases/latest/download/${filename}`;
				const targetPath = path.join(ctx.versionedDir, filename);
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @oh-my-gpt/gpt-natives\n" +
...
function initLoaderContext() {
	...
	const userDataDir =
		process.platform === "win32"
			? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "omg")
	...
	// `scripts/release.ts` bumps the name in `crates/gpt-natives/src/lib.rs` in
