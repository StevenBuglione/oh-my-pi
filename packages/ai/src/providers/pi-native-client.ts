 ...
 *
 * Dispatches a {@link streamSimple}-shaped request to an `omg auth-gateway`
 ...
 * containerized omg deployments (robomp slots, the swarm extension) that
 * route every LLM call through a credential-holding sidecar so the slot
 * itself stays credential-free.
 */
import { readSseJson } from "@oh-my-gpt/gpt-utils";
import type {
	Api,
	AssistantMessage,
...
/**
 * Stream a turn through an `omg auth-gateway` over the pi-native protocol.
 *
