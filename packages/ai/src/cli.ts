...
async function login(provider: OAuthProvider): Promise<void> {
	...
	try {
		...
		console.log(`\nCredentials saved to ~/.omg/agent/agent.db`);
	} finally {
		store.close();
		rl.close();
	}
...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		console.log(`Usage: bunx @oh-my-gpt/gpt-ai <command> [provider]
...
Examples:
  ...
  bunx @oh-my-gpt/gpt-ai logout anthropic   # logout from specific provider
  bunx @oh-my-gpt/gpt-ai status             # show logged-in providers
  bunx @oh-my-gpt/gpt-ai list               # list providers
`);
		return;
	}
	...
	if (command === "status") {
		...
		try {
			...
			if (providers.length === 0) {
				...
				console.log(`Use 'bunx @oh-my-gpt/gpt-ai login' to authenticate.`);
			} else {
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some(p => p.id === provider)) {
			...
			console.error(`Use 'bunx @oh-my-gpt/gpt-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'bunx @oh-my-gpt/gpt-ai --help' for usage`);
	process.exit(1);
}
