...
3. `getSSHConfigPath("project")` and `getSSHConfigPath("user")` in `packages/utils/src/dirs.ts` resolve those managed files to `.omg/ssh.json` in the project and `~/.omg/agent/ssh.json` in the user config dir. This tool does not read `~/.ssh/config`.
...
- Subprocesses / native bindings
  ...
  - Sanitizes streamed text with `@oh-my-gpt/gpt-natives` text sanitization.
