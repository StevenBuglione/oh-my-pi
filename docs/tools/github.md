...
- Worktree path is `path.join(getWorktreesDir(), encodeRepoPathForFilesystem(primaryRepoRoot), localBranch)`, where `getWorktreesDir()` is `~/.omg/wt`; effective path is `~/.omg/wt/<encoded-primary-repo-root>/pr-<number>`.
...
- The branch push metadata is persisted with `git config` under the repository's shared `.git/config` as:
  ...
  - `branch.pr-<number>.omgPrHeadRef`
  - `branch.pr-<number>.omgPrUrl`
  - `branch.pr-<number>.omgPrIsCrossRepository`
  - `branch.pr-<number>.omgPrMaintainerCanModify`
...
Push target resolution reads the `branch.<name>.omgPrHeadRef`, `pushRemote`/`remote`, `ompPrUrl`, `ompPrMaintainerCanModify`, and `ompPrIsCrossRepository` git-config keys written by `pr_checkout`. If the current checked-out branch matches the target branch, the source ref is `HEAD`; otherwise it pushes `refs/heads/<branch>`. The refspec is `HEAD:refs/heads/<headRef>` or `refs/heads/<branch>:refs/heads/<headRef>`.
...
- Filesystem
  ...
  - `pr_checkout` may create directories under `~/.omg/wt/<encoded-primary-repo-root>/` and add git worktrees there.
