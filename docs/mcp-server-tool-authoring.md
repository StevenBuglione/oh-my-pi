...
```text
Config sources (.omg/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.omg/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.omg/mcp.json` (project) or `~/.omg/agent/mcp.json` (user) for explicit control.
