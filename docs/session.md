...
```text
~/.omg/agent/sessions/--<cwd-encoded>--/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.omg/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.omg/agent/terminal-sessions/<terminal-id>
```
...
- `listAll()` -> sessions across all project scopes under `~/.omg/agent/sessions`
