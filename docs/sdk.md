...
The SDK is the in-process integration surface for `@oh-my-gpt/gpt-coding-agent`.
...
```bash
bun add @oh-my-gpt/gpt-coding-agent
```

## Entry points

`@oh-my-gpt/gpt-coding-agent` exports the SDK APIs from the package root (and also via `@oh-my-gpt/gpt-coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@oh-my-gpt/gpt-coding-agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.omg/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@oh-my-gpt/gpt-coding-agent";
...
```ts
import { SessionManager } from "@oh-my-gpt/gpt-coding-agent";
