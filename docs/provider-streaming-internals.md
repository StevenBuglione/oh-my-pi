...
This document explains how token/tool streaming is normalized in `@oh-my-gpt/gpt-ai`, then propagated through `@oh-my-gpt/gpt-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@oh-my-gpt/gpt-ai`
