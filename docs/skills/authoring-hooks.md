...
description: Use when creating a new omg hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---
...
```ts
import type { HookAPI } from "@oh-my-gpt/gpt-coding-agent/extensibility/hooks";

export default function myHook(omg: HookAPI): void {
  omg.on("tool_call", async (event, ctx) => {
...
```ts
omg.on("tool_result", async (event, ctx) => {
...
```ts
omg.on("context", async (event, ctx) => {
...
export default function rmRfBlocker(omg: HookAPI): void {
  omg.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(omg: HookAPI): void {
  omg.on("tool_result", async (event) => {
...
export default function contextFilter(omg: HookAPI): void {
  omg.on("context", async (event) => {
